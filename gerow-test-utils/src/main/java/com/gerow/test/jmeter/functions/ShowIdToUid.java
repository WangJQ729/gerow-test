...
public class ShowIdToUid extends AbstractFunction {
    private static final List<String> desc = List.of("showId");
    private static final String KEY = "__showIdToUid";
    private CompoundVariable showId;
    ...
    public void setParameters(Collection<CompoundVariable> parameters) throws InvalidVariableException {
        ...
        showId = parameters.iterator().next();
    }

    public static int showIdToUid(long showId) {
        for (int i = 10000; i <= 100000000; i++) {
            long userShowId = getUserShowId(i);
            if (showId == userShowId) {
                return i;
            }
        }
        return 0;
    ...
    public String execute(SampleResult previousResult, Sampler currentSampler) throws InvalidVariableException {
        long showIdValue = Long.parseLong(showId.execute().trim());
        Integer uid = showIdToUid(showIdValue);
